  ...
  return (
    <Sheet open={meetingId != null} onOpenChange={onOpenChange}>
      <SheetContent className="w-full gap-0 overflow-y-auto p-0 sm:max-w-lg">
        {meetingId && <Body meetingId={meetingId} title={title} />}
      </SheetContent>
    </Sheet>
  );
}

function Body({ meetingId, title }: { meetingId: string; title: string }) {
  ...
  const loadAdded = useCallback(async () => {
    try {
      const d = await api.get<{ students: Student[] }>(
        `/api/meetings/${meetingId}/students`,
      );
      setAdded(d.students);
    } catch {
      setAdded([]);
  ...
  useEffect(() => {
    let alive = true;
    api
      .get<{ students: Student[] }>(`/api/meetings/${meetingId}/students`)
    ...
    const t = setTimeout(() => {
      ...
      api
        .get<{ students: Student[] }>(
          `/api/users/students${search ? `?search=${encodeURIComponent(search)}` : ""}`,
        )
        .then((d) => alive && setResults(d.students))
  ...
  async function remove(userId: string) {
    try {
      await api.del(`/api/meetings/${meetingId}/students/${userId}`);
  ...
  return (
    <div className="flex flex-col">
      <SheetHeader className="border-b p-6 pb-4">
        ...
        <SheetDescription>
          Pick the learners attending this class. They&apos;ll appear on the
          attendance roster and can open the video link.
        </SheetDescription>
      </SheetHeader>
      ...
      <div className="border-b px-6 py-4">
        <p className="mb-3 flex items-center gap-2 text-sm font-medium">
          ...
          On this class
          {added && (
            <span className="text-muted-foreground font-normal">({added.length})</span>
          )}
        </p>
        {!added ? (
        ...
        <div className="mt-3 divide-y">
          {searching && results.length === 0 ? (
            ...
            <p className="text-muted-foreground py-6 text-center text-sm">
              {search ? "No matching students." : "Everyone found is already added."}
            </p>
          ) : (
            selectable.map((s) => (
              <label
